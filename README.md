# Fair, Physics-Validated ML Inverse Design of High-Rate Lithium-Ion Electrodes

This repository contains the complete dataset and code to reproduce the results of the paper:

> **A Fair, Physics-Validated Surrogate Framework for High-Rate Lithium-Ion Electrode Design**
> *[Author names]* ([journal / DOI to be added])
...
Key packages: `pybamm`, `xgboost`, `scikit-learn`, `shap`, `scipy`, `numpy`, `pandas`, `matplotlib` (exact versions pinned in `requirements.txt`).
...
```bibtex
@article{[key],
  title   = {A Fair, Physics-Validated Surrogate Framework for High-Rate Lithium-Ion Electrode Design},
  author  = {[Authors]},
  journal = {[Journal]},
  year    = {[Year]},
  doi     = {[DOI]}
